...
- 资产详情最近历史图使用稀疏日期横轴和数值纵轴；比价分历史标出 `120`、`100`、
  `80` 阈值，并在 `100` 绘制基准横线。
- Opportunities tab 同页展示强势/候选多头和强势/候选空头结果。每组先按规则筛出全部机会并显示总数，
...
- 资产详情只生成四种信号标签，且逐条独立判断、可同时出现多个：`高置信多头` 要求趋势分、
  比价强度分和杠杆速率分均 `≥ 70`；`高置信空头` 要求三者均 `≤ -70`；`快速加杠杆` 要求
...
- 候选多头要求 `current_relative_state == "Improving"`、`early_reversal > 100`、
  ...
  tie-breaker 排列。
- 强势空头要求 `current_relative_state` 为 `Weakening` 或 `Lag`、`early_reversal < 100`、
  资金状态为 `Deleveraging`、`trend_score > 20` 且日趋势为 `down`。排序依次使用去杠杆状态
  持续时间升序、杠杆值降序和稳定资产身份。
- 候选空头要求 `current_relative_state` 为 `Weakening` 或 `Lag`、`early_reversal < 100`、
