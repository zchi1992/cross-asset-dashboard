...
- Market Map 和交易机会为同页 tab，共用当前日期和底部播放控件；“持仓”是独立 tab，
  使用最新 IBKR 本地快照且不显示 playback 控件。
- 后端不可访问时显示可执行的启动提示。
...
- “持仓”页显示账户摘要、风险指标和完整持仓，并提供“同步”按钮。同步期间按钮禁用；失败时
  保留最近一次成功快照并显示可执行错误。点击已关联持仓会回到最新 Market Map 日期并打开
  现有资产详情。
- 持仓明细将 `OPT`、`FOP` 按底层 symbol 合并为可展开父行，父行汇总合约数、市值、未实现盈亏
  和权重；展开后显示各期权合约。表格各列支持升降序排列，缺失值固定排在末尾。
- 持仓明细包含“概览”和“期权”子页面。期权页按底层 symbol 汇总 Delta、Gamma、Theta、Vega；
  每项敞口使用 `IBKR model Greek × signed quantity × multiplier`，同方向相加、反方向抵消。任一
  合约缺少该项 Greek 或可靠 multiplier 时，该底层标的对应汇总显示“无法计算”。底层标的行可
  展开，子行逐张展示合约数量和各项 Greek 敞口，并与父行汇总一致。
- 新增用户可见文案使用中文，内部 API、TypeScript 类型和 CSV 字段继续使用英文。

## 持仓与风控

- 账户概览显示净清算价值、维持保证金、SMA、超额流动性、可用资金、购买力、持仓总价值和
  Cushion。
- 风控显示 `GrossPositionValue / NetLiquidation` 杠杆率、维持保证金率、超额流动性率、
  最大单一持仓集中度，以及已设置止损头寸的成本至止损亏损和当前剩余风险。
- 未填写 Stop Loss 的非期权头寸不计算风险；`OPT`、`FOP`、`BAG` 展示持仓但不提供止损，
  也不进入覆盖率分母。
- 止损风险为计划估算，不承诺成交价格。无法可靠换算乘数或币种时显示“无法计算”。
